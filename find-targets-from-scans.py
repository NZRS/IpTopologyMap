...
parser.add_argument('--topology-data', required=True,
                    help="File with the list of AS Paths and prefixes we want to look for")
...
with open(args.topology_data, 'rb') as prefix_data:
    aspath = json.load(prefix_data)
    for path in aspath['aspath']:
        for prefix in path['prefixes']:
            cc_prefix_list.add(prefix)

"""Build the radix tree"""
rt = Radix()
for p1 in cc_prefix_list:
...
print "%d prefixes" % (len(cc_prefix_list))
selected_addr = []
